...
.Duplicated Imports
====
The HTML imports we have in the [filename]#index.html# file also import any possible dependencies. Imports having the same dependencies result in multiple imports of the same file. In such case, bowsers prevent fetching the same file multiple times by checking the file location.
