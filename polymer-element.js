...
function PolymerElement(name) {
    ...
    var notifyForDiffersDirective = core_1.Directive({
        ...
        _handleArrayDiffs: function (property, diff) {
            if (diff) {
                var elm = this._element;
                diff.forEachRemovedItem(function (item) { return elm.notifyPath(property + '.' + item.previousIndex); });
                diff.forEachAddedItem(function (item) { return elm.notifyPath(property + '.' + item.currentIndex); });
                diff.forEachMovedItem(function (item) { return elm.notifyPath(property + '.' + item.currentIndex); });
            }
        },
